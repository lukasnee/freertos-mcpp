...
#ifndef __FREERTOS_CONDITION_VARIABLE_HPP_
#define __FREERTOS_CONDITION_VARIABLE_HPP_
...
class condition_variable_any
{
  public:
    ...
    /// @brief  Atomically unlocks @ref lock, and blocks the thread until a notification is
    /// received.
    ...
    cv_status wait_for(Lock& lock, const std::chrono::duration<Rep, Period>& rel_time)
    {
        ...
        bool success =
            do_wait(std::chrono::duration_cast<tick_timer::duration>(rel_time), &rx_waiters);
  ...
  private:
    using waiter_count_t = ::UBaseType_t;
...
#endif // __FREERTOS_CONDITION_VARIABLE_HPP_
