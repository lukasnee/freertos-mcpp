...
#ifndef __FREERTOS_SEMAPHORE_HPP_
#define __FREERTOS_SEMAPHORE_HPP_
...
class semaphore : protected queue
{
  public:
    ...
    inline void acquire() { (void)take(infinity); }
    ...
    inline bool try_acquire() { return take(tick_timer::duration(0)); }
    ...
    /// @remark Thread and ISR context callable
    inline void release(count_type update = 1) { (void)give(update); }

    /// @brief  Function to observe the semaphore's current acquirable count.
    /// @return The semaphore's acquirable count
    /// @remark Thread and ISR context callable
    inline count_type get_count() const { return queue::size(); }
...
class counting_semaphore : public semaphore
{
  public:
    ...
    static constexpr count_type max() { return MAX_VALUE; }

    /// @brief  Constructs a counting semaphore statically.
    counting_semaphore(count_type desired = 0) : semaphore(MAX_VALUE, desired) {}
};
...
class binary_semaphore : public semaphore
{
  public:
    /// @brief  Maximum value of the internal counter.
    static constexpr count_type max() { return 1; }
...
#endif // __FREERTOS_SEMAPHORE_HPP_
