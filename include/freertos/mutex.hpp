...
#ifndef __FREERTOS_MUTEX_HPP_
#define __FREERTOS_MUTEX_HPP_
...
class mutex : protected semaphore
{
  public:
    ...
    inline thread* get_locking_thread() const { return semaphore::get_mutex_holder(); }
...
class recursive_mutex : protected semaphore
{
  public:
    ...
    /// @remark Thread context callable
    inline void lock() { (void)recursive_take(infinity); }
    ...
    /// @remark Thread context callable
    inline bool try_lock() { return recursive_take(tick_timer::duration(0)); }

    /// @brief  Reduces the thread's lock count and unlocks the mutex when the lock count reaches
    /// zero.
    /// @remark Thread context callable
    void unlock();
...
/// @brief  A type alias since @ref recursive_mutex already implements std::recursive_timed_mutex
/// API.
